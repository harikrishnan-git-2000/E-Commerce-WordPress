...
class Solace_Extra_Nav_Menu extends \Elementor\Widget_Base {
	...
	public function get_script_depends() {
		return [ 'solace-nav-menu', 'solace-smartmenus' ];
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'menu_name',
			[
				'label' => esc_html__( 'Menu Name', 'solace-extra' ),
				'type' => Controls_Manager::TEXT,
				'default' => esc_html__( 'Menu', 'solace-extra' ),
			]
		);
		...
		if ( ! empty( $menus ) ) {
			$this->add_control(
				'menu',
				[
					'label' => esc_html__( 'Menu', 'solace-extra' ),
					...
					'description' => sprintf(
						...
						esc_html__( 'Go to the %1$sMenus screen%2$s to manage your menus.', 'solace-extra' ),
			...
			$this->add_control(
				...
				[
					...
					'alert_type' => 'info',
					'heading' => esc_html__( 'There are no menus in your site.', 'solace-extra' ),
					'content' => sprintf(
						/* translators: 1: Link opening tag, 2: Link closing tag. */
						esc_html__( 'Go to the %1$sMenus screen%2$s to create one.', 'solace-extra' ),
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'dropdown' => esc_html__( 'Dropdown', 'solace-extra' ),
				],
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					'start' => [
						'title' => esc_html__( 'Start', 'solace-extra' ),
					...
					'end' => [
						'title' => esc_html__( 'End', 'solace-extra' ),
		...
		$this->add_control(
			'pointer',
			[
				'label' => esc_html__( 'Pointer', 'solace-extra' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'underline',
				'options' => [
					'none' => esc_html__( 'None', 'solace-extra' ),
					'underline' => esc_html__( 'Underline', 'solace-extra' ),
					'overline' => esc_html__( 'Overline', 'solace-extra' ),
					'double-line' => esc_html__( 'Double Line', 'solace-extra' ),
					'framed' => esc_html__( 'Framed', 'solace-extra' ),
					'background' => esc_html__( 'Background', 'solace-extra' ),
					'text' => esc_html__( 'Text', 'solace-extra' ),
				],
				'style_transfer' => true,
				'condition' => [
					'layout!' => 'dropdown',
		...
		$this->add_control(
			'animation_line',
			[
				'label' => esc_html__( 'Animation', 'solace-extra' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'fade',
				'options' => [
					'fade' => 'Fade',
		...
		$this->add_control(
			'submenu_icon',
			[
				'label' => esc_html__( 'Submenu Indicator', 'solace-extra' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Mobile Dropdown', 'solace-extra' ),
		...
		foreach ( $breakpoints as $breakpoint_key => $breakpoint_instance ) {
			...
			$dropdown_options[ $breakpoint_key ] = sprintf(
				...
				esc_html__( '%1$s (%2$s %3$dpx)', 'solace-extra' ),
				$breakpoint_instance['label'],
				'>',
				$breakpoint_instance['default_value']
			);
		}

		$dropdown_options['none'] = esc_html__( 'None', 'solace-extra' );

		$this->add_control(
			'dropdown',
			[
				'label' => esc_html__( 'Breakpoint', 'solace-extra' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'tablet',
		...
		$this->add_control(
			'full_width',
			[
				'label' => esc_html__( 'Full Width', 'solace-extra' ),
				'type' => Controls_Manager::SWITCHER,
				'description' => esc_html__( 'Stretch the dropdown of the menu to full width.', 'solace-extra' ),
		...
		$this->add_control(
			'text_align',
			[
				'label' => esc_html__( 'Text  Align', 'solace-extra' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'aside',
				'options' => [
					'aside' => esc_html__( 'Aside', 'solace-extra' ),
					'center' => esc_html__( 'Center', 'solace-extra' ),
				],
		...
		$this->add_control(
			'toggle',
			[
				'label' => esc_html__( 'Toggle Button', 'solace-extra' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'burger',
				'options' => [
					'' => esc_html__( 'None', 'solace-extra' ),
					'burger' => esc_html__( 'Hamburger', 'solace-extra' ),
				],
		...
		$this->add_control(
			'toggle_icon_normal',
			[
				'label' => esc_html__( 'Icon', 'solace-extra' ),
				'type' => Controls_Manager::ICONS,
				...
				'skin_settings' => [
					'inline' => [
						'none' => [
							'label' => esc_html__( 'Default', 'solace-extra' ),
							'icon' => 'eicon-menu-bar',
						],
						'icon' => [
							'icon' => 'eicon-star',
		...
		$this->add_control(
			'toggle_align',
			[
				'label' => esc_html__( 'Toggle Align', 'solace-extra' ),
				'type' => Controls_Manager::CHOOSE,
				'default' => 'center',
				'options' => [
					'left' => [
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Pointer Color', 'solace-extra' ),
				'type' => Controls_Manager::COLOR,
				'global' => [
					'default' => Global_Colors::COLOR_ACCENT,
				],
		...
		$this->add_control(
			'nav_menu_divider',
			[
				'label' => esc_html__( 'Divider', 'solace-extra' ),
				'type' => Controls_Manager::SWITCHER,
				'label_off' => esc_html__( 'Off', 'solace-extra' ),
				'label_on' => esc_html__( 'On', 'solace-extra' ),
				'condition' => [
					'layout' => 'horizontal',
				],
				'selectors' => [
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::SELECT,
				'options' => [
					'solid' => esc_html__( 'Solid', 'solace-extra' ),
					'double' => esc_html__( 'Double', 'solace-extra' ),
					'dotted' => esc_html__( 'Dotted', 'solace-extra' ),
					'dashed' => esc_html__( 'Dashed', 'solace-extra' ),
				],
				'default' => 'solid',
		...
		$this->add_responsive_control(
			'pointer_width',
			[
				'label' => esc_html__( 'Pointer Width', 'solace-extra' ),
				'type' => Controls_Manager::SLIDER,
		...
		$this->add_responsive_control(
			...
			[
				'label' => esc_html__( 'Horizontal Padding', 'solace-extra' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'custom' ],
				'range' => [
					'px' => [
		...
		$this->add_responsive_control(
			...
			[
				'label' => esc_html__( 'Vertical Padding', 'solace-extra' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'custom' ],
				'range' => [
					'px' => [
		...
		$this->add_responsive_control(
			'menu_space_between',
			[
				'label' => esc_html__( 'Space Between', 'solace-extra' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'custom' ],
				'range' => [
					'px' => [
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Dropdown', 'solace-extra' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_control(
			...
			[
				'raw' => esc_html__( 'On desktop, this will affect the submenu. On mobile, this will affect the entire menu.', 'solace-extra' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Border Width', 'solace-extra' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', '%', 'em', 'rem', 'vw', 'custom' ],
				'range' => [
					'px' => [
		...
		$this->add_responsive_control(
			'toggle_size',
			[
				'label' => esc_html__( 'Size', 'solace-extra' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'custom' ],
				'range' => [
					'px' => [
	...
	public function get_frontend_settings() {
		...
		if ( 'fa ' === substr( $frontend_settings['submenu_icon']['value'], 0, 3 ) ) {
	...
	/**
	 * Handle additional link classes for menu items.
	 */
	public function add_span_to_menu_items( $item_output, $item, $depth, $args ) {
		$frontend_settings = parent::get_frontend_settings();
		...
		if ( in_array( 'menu-item-has-children', $item->classes ) ) {
			...
			// Insert the icon markup before closing </a> tag.
			$item_output = str_replace( '</a>', $icon_markup . '</a>', $item_output );
		}
		...
		if ( in_array( 'menu-item-has-children', $item->classes ) ) {
			...
			// Insert the icon markup after the closing </a> tag.
			$item_output = $item_output . $icon_markup;
		}
	...
	protected function render() {
		// Get available menus. If none are available, stop rendering.
		add_filter( 'walker_nav_menu_start_el', [ $this, 'add_span_to_menu_items' ], 10, 4 );
		...
		?>
			<nav class="elementor-nav-menu--dropdown solace-elementor-nav-menu--dropdown elementor-nav-menu__container" aria-hidden="true">
				<?php
					// PHPCS - escaped by WordPress with "wp_nav_menu"
	...
	private function render_menu_toggle( $settings ) {
		...
		$this->add_render_attribute( 'menu-toggle', [
			'class' => 'elementor-menu-toggle solace-elementor-menu-toggle',
			'role' => 'button',
			'tabindex' => '0',
			'aria-label' => esc_html__( 'Menu Toggle', 'solace-extra' ),
			'aria-expanded' => 'false',
		] );
		...
		<div <?php $this->print_render_attribute_string( 'menu-toggle' ); ?>>
			...
			?>
			<span class="elementor-screen-only"><?php echo esc_html__( 'Menu', 'solace-extra' ); ?></span>
		</div>
		<?php
	}
