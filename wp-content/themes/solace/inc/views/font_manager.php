...
class Font_Manager extends Base_View {
	...
	private function add_body_variants() {
		$body_font   = self::get_body_font_family();
		$gfonts_data = solace_get_google_fonts( true );
	...
	private function enqueue_google_font( $font, $weights = [], $skip_enqueue = false ) {
		...
		$google_fonts = solace_get_google_fonts( true );
	...
	public function load_external_fonts_locally() {
		 ...
		 * @since 2.11
		 */
		$should_enqueue_locally = apply_filters( 'solace_load_remote_fonts_locally', $toggle );
		 ...
		 *
		 * We do this so that initial load of Elementor editor will not be slowed down by Solace Pro downloading Roboto fonts.
		 */
		...
		foreach ( $external_fonts as $font_link ) {
			wp_add_inline_style( 'solace-style', wptt_get_webfont_styles( $font_link ) );
		}
