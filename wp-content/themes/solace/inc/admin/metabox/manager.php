...
use Solace\Views\Post_Layout;
use Solace\Core\Supported_Post_Types;
...
final class Manager {
	...
	public function define_controls() {
		$this->control_classes = array(
			'Solace\\Admin\\Metabox\\Main',
		);

		$this->control_classes = apply_filters( 'solace_filter_metabox_controls', $this->control_classes );
	}
	...
	public function add() {
		...
		add_meta_box(
			'solace-page-settings',
			sprintf(
			/* translators: %s - post type */
				__( '%s Settings', 'solace' ),
				$post_type
			),
	...
	// 	echo '<h4>' . esc_html__( 'Page Settings are now accessible from the top bar', 'solace' ) . '</h4>';
	// 	printf(
	// 	/* translators: %1$s - Keyboard shortcut.   %2&s - svg icon */
	// 		esc_html__( 'Click the %1$s icon in the top bar or use the keyboard shortcut ( %2$s ) to customise the layout settings for this page', 'solace' ),
	...
	// 			</svg>',
	// 		'<strong>SHIFT + ALT + S</strong> ' . esc_html__( 'or', 'solace' ) . ' <strong>control + option + S</strong>'
	// 	);
	// 	echo '</div>';
	// }
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'solace-metabox' );
	 ...
	 * Register meta
	 */
	public function solace_register_meta() {
		$meta_sidebar_controls = apply_filters(
			'solace_sidebar_meta_controls',
			[
				[
					'id'   => 'solace_meta_sidebar',
					'type' => 'radio',
				],
				[
					'id'   => 'solace_meta_container',
					'type' => 'button-group',
				],
				[
					'id'   => 'solace_meta_enable_content_width',
					'type' => 'checkbox',
				],
				[
					'id'   => 'solace_meta_content_width',
					'type' => 'range',
				],
				[
					'id'   => 'solace_meta_title_alignment',
					'type' => 'button-group',
				],
				[
					'id'   => 'solace_meta_author_avatar',
					'type' => 'checkbox',
				],
				[
					'id'   => 'solace_post_elements_order',
					'type' => 'sortable-list',
				],
				[
					'id'   => 'solace_meta_disable_header',
					'type' => 'checkbox',
				],
				[
					'id'   => 'solace_meta_disable_footer',
					'type' => 'checkbox',
				],
				[
					'id'   => 'solace_meta_disable_title',
					'type' => 'checkbox',
				],
			]
		);
	...
	public function meta_sidebar_script_enqueue() {
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'solace-meta-sidebar', 'solace' );
		}
		...
		$advanced_layout = Mods::get( Config::MODS_ADVANCED_LAYOUT_OPTIONS, solace_is_new_skin() );
		...
		$localized_data = apply_filters(
			'solace_meta_sidebar_localize_filter',
			array(
				'actions'              => array(
					'solace_meta_content_width' => array(
						'container' => $container,
						'editor'    => $editor_width,
	...
	private function get_author_avatar_state() {
		$show_avatar = get_theme_mod( 'solace_author_avatar', false );
		return get_theme_mod( 'solace_single_post_author_avatar', $show_avatar );
	}
	...
	public function set_page_width( $post_id, $post ) {
		...
		$checkout_was_updated = get_post_meta( $post_id, 'solace_checkout_updated', true );
		...
		if ( Main::is_new_page() || ( Main::is_checkout() && $checkout_was_updated === 'no' ) ) {
			update_post_meta( $post_id, 'solace_meta_sidebar', 'full-width' );
			update_post_meta( $post_id, 'solace_meta_enable_content_width', 'on' );
			update_post_meta( $post_id, 'solace_meta_content_width', 100 );
			if ( Main::is_checkout() ) {
				update_post_meta( $post_id, 'solace_checkout_updated', 'yes' );
			}
		}
	}
}
