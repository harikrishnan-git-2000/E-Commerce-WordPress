...
class Lifter {
	...
	private function load_hooks() {
		...
		add_action( 'solace_llms_content', array( $this, 'content_open' ), 10 );
		add_action(
			'lifterlms_loop',
			function() {
				echo '</div>';
		...
		add_filter(
			'solace_lifter_sidebar_setup',
			function() {
				return [
					'theme_mod' => 'solace_default_sidebar_layout',
	...
	private function get_sidebar_position() {
		$advanced_sidebar = get_theme_mod( 'solace_advanced_layout_options', false );
		$sidebar_position = get_theme_mod( 'solace_default_sidebar_layout', 'right' );
		if ( $advanced_sidebar === true ) {
			$sidebar_position = get_theme_mod( 'solace_other_pages_sidebar_layout', 'right' );
		}
	...
	public function load_catalog_sidebar() {
		...
		if ( $sidebar_position === 'right' ) {
			add_action( 'solace_llms_content_after', array( $this, 'render_catalog_sidebar' ), 11 );
		}
		if ( $sidebar_position === 'left' ) {
			add_action( 'solace_llms_content', array( $this, 'render_catalog_sidebar' ), 1 );
		}
	}
	...
	public function content_wrapper_open() {
		$container_class = apply_filters( 'solace_container_class_filter', 'container' );
		echo '<div class="' . esc_attr( $container_class ) . ' lms-container">';
		echo '<div class="row">';
		do_action( 'solace_llms_content' );
	}
	...
	public function content_wrapper_close() {
		do_action( 'solace_llms_content_after' );
		echo '</div>';
	}
	...
	public function content_open() {
		...
		echo '</div>';
		echo '</div>';
		$class = apply_filters( 'solace_lifter_wrap_classes', 'nv-content-wrap entry-content' );
	...
	public function register_catalog_sidebar() {
		register_sidebar(
			array(
				'name'          => __( 'Catalog Sidebar', 'solace' ),
				'id'            => 'llms_shop',
	...
	public function render_catalog_sidebar() {
		...
		do_action( 'solace_before_sidebar_content', 'lifter', $sidebar_position );

		$has_custom_sidebar = apply_filters( 'solace_has_custom_sidebar', false, 'lifter' );

		if ( ! $has_custom_sidebar ) {
			dynamic_sidebar( 'llms_shop' );
		}

		do_action( 'solace_after_sidebar_content', 'lifter', $sidebar_position );
