...
class Css_Prop {
	...
	public static function get_suffix_responsive( $meta, $device ) {
		...
		// The solace_responsive_range_control component double JSON stringified, therefore try to parse it again.
	...
	public static function transform( $css_prop, $value, $meta, $device ) {
		...
		switch ( $css_prop ) {
			...
			case Config::CSS_PROP_BORDER_COLOR:
				...
				return sprintf( "%s: %s%s;", ( $css_prop ), solace_sanitize_colors( $value ), isset( $meta['important'] ) && $meta['important'] ? '!important' : '' );
	...
	public static function transform_directional_prop( $meta, $device, $value, $css_prop, $type ) {
		...
		if ( count( array_unique( $filtered ) ) === 1 ) {
			...
			if ( solace_value_is_zero( $value['top'] ) ) {
				$suffix = '';
			}

			if ( empty( $value['top'] ) && ! solace_value_is_zero( $value['top'] ) ) {
				return '';
			}
		...
		if ( count( array_unique( $filtered ) ) === 2 && $value['top'] === $value['bottom'] && $value['right'] === $value['left'] ) {
			...
			if ( solace_value_is_zero( $value['top'] ) && solace_value_is_zero( $value['right'] ) ) {
				return '';
			}

			$top_suffix   = solace_value_is_zero( $value['top'] ) ? '' : $suffix;
			$right_suffix = solace_value_is_zero( $value['right'] ) ? '' : $suffix;
		...
		foreach ( Config::$directional_keys as $direction ) {
			if ( ! isset( $value[ $direction ] ) || solace_value_is_zero( $value[ $direction ] ) ) {
				$template .= '0 ';
	...
	public static function format_font_family_value( $value ) {
		...
		if ( strpos( $value, ',' ) !== false ) {
			...
			$value = array_map( 'Solace\Core\Styles\CSS_Prop::quote_font_family', $value );
