...
class Customizer {
	...
	public function __construct() {
		...
		add_filter( 'solace_react_controls_localization', array( $this, 'add_builders_and_dynamic_tags' ) );
	}
	...
	public function register( WP_Customize_Manager $wp_customize ) {
		...
		$wp_customize->register_section_type( '\Solace\Customizer\Controls\React\Instructions_Section' );
		$wp_customize->register_section_type( '\Solace\Customizer\Controls\React\Typography_Extra_Section' );
					...
					<div class="hfg--cb-header">
						...
						<div class="hfg--cb-notice conditional-header hidden">
							...
							<p>
								...
								echo wp_kses_post( sprintf( __( 'You are customizing the %s Header', 'solace' ), ' <a>' . __( 'Default', 'solace' ) . '</a> ' ) );
								?>
							</p>
						</div>
						<# } #>
						<div class="hfg--cb-notice welcome-notice {{data.id}} hidden">
							<p>
								...
								<span><?php echo sprintf( esc_html__( '%s Builder:', 'solace' ), '{{data.title}}' ); ?></span>
								<?php
								/* translators: %s is the header name */
								echo esc_html__( 'Click on any empty space to add components, or existing components to adjust settings.', 'solace' );
								?>
						...
						<div class="hfg--cb-actions">
							...
							<a class="button button-secondary hfg--panel-close" href="#">
								<span class="close-text"><i class="dashicons dashicons-arrow-down-alt2"
															style="margin-top: 4px;"></i> <?php esc_html_e( 'Close', 'solace' ); ?></span>
		...
		<script type="text/html" id="tmpl-hfg--cb-item">
			...
			>
				...
				<div class="grid-stack-item-content">
					...
					<# if(data.section.indexOf('solace_') !== -1) { #>
				...
				<div class="hfg-widgets-panel-header">
					<div class="hfg-component-search">
						...
						<input class="component-search" type="search"
								placeholder="<?php esc_attr_e( 'Search Components', 'solace' ); ?>..."/>
					</div>
