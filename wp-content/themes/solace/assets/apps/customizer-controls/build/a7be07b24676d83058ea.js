...
const SidebarContent = ({ items }) => {
    ...
    // sortedItems [
    // 	{"id":"footer_account","name":"Account"},
    // 	{"id":"button_base3","name":"Button One"},
    // 	{"id":"button_base4","name":"Button Two"},
    // 	{"id":"footer_cart_icon","name":"Cart"},
    // 	{"id":"footer_contact","name":"Contacts"},
    // 	{"id":"footer-four-widgets","name":"Footer Four"},
    // 	{"id":"footer-menu","name":"Footer Menu"},
    // 	{"id":"footer-one-widgets","name":"Footer One"},
    // 	{"id":"footer-three-widgets","name":"Footer Three"},
    // 	{"id":"footer-two-widgets","name":"Footer Two"},
    // 	{"id":"footer_html1","name":"HTML One"},
    // 	{"id":"footer_html3","name":"HTML Three"},
    // 	{"id":"footer_html2","name":"HTML Two"},
    // 	{"id":"logo-footer","name":"Logo"},
    // 	{"id":"footer_search","name":"Search Form"}
    // ]
    // Find index of the item to move
    const footerTwoWidgets = sortedItems.findIndex(item => item.id === 'footer-two-widgets');
    const footerHtml2 = sortedItems.findIndex(item => item.id === 'footer_html2');
    // Move the item to index 8 if it's found and it's currently at index 9
    if (footerTwoWidgets !== -1 && footerTwoWidgets === 9) {
        const [footerTwoWidgetsItem] = sortedItems.splice(footerTwoWidgets, 1);
        sortedItems.splice(8, 0, footerTwoWidgetsItem);
    }
    // Move the item to index 11 if it's found and it's currently at index 12
    if (footerHtml2 !== -1 && footerHtml2 === 12) {
        const [footerHtml2Item] = sortedItems.splice(footerHtml2, 1);
        sortedItems.splice(11, 0, footerHtml2Item);
    }
    return (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "solace-builder-sidebar-content" },
        ...
        items && items.length > 0 && (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "sidebar-items droppable-wrap" },
            ...
            , { 
                ...
                // onChoose={handleChoose}
                // Sidebar on drop
                onSort: handleChoose, onEnd: onDragEnd, animation: 0, group: builder, className: `droppable`, disabled: dragging, onStart: onDragStart, list: items, setList: (next) => {
                ...
                return (react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", { className: "builder-item", key: item.id },
...
//# sourceMappingURL=a7be07b24676d83058ea.js.map